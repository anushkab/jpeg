 ...
 */
static int
pgetc (unsigned char CurHuffReadBuf[JPEG_FILE_SIZE])
{
	int temp;
 ...
 */
int
buf_getb (unsigned int * current_read_byte, int * read_position, unsigned char CurHuffReadBuf[JPEG_FILE_SIZE])
{
	const int bit_set_mask[32] = {	/* This is 2^i at ith position */
			0x00000001, 0x00000002, 0x00000004, 0x00000008,
			0x00000010, 0x00000020, 0x00000040, 0x00000080,
	...
	if (*read_position < 0)
	{
		*current_read_byte = pgetc (CurHuffReadBuf);
		*read_position = 7;
	}
 ...
 */
int
buf_getv (int n, unsigned int * current_read_byte, int * read_position, unsigned char CurHuffReadBuf[JPEG_FILE_SIZE])
{
	int p, rv;
	...
	while (p > 0)
	{
		...
		{			/* If byte buffer contains almost entire word */
			...
			*current_read_byte = pgetc (CurHuffReadBuf);	/* Change read bytes */
		...
		*current_read_byte = (*current_read_byte << 8) | pgetc (CurHuffReadBuf);
 ...
 */
int
huff_make_dhuff_tb (int p_xhtbl_bits[36], int p_dhtbl_ml,
		int p_dhtbl_maxcode[36], int p_dhtbl_mincode[36],
		int p_dhtbl_valptr[36])
{
 ...
 */
int
DecodeHuffman (int Xhuff_huffval[257], int Dhuff_ml,
		int Dhuff_maxcode[36], int Dhuff_mincode[36], int Dhuff_valptr[36], int * main_result, unsigned int * current_read_byte, int * read_position, unsigned char CurHuffReadBuf[JPEG_FILE_SIZE])
{
	int code, l, p;

	code = buf_getb (current_read_byte, read_position, CurHuffReadBuf);
	for (l = 1; code > Dhuff_maxcode[l]; l++)
	{
		code = (code << 1) + buf_getb (current_read_byte, read_position, CurHuffReadBuf);
	}
